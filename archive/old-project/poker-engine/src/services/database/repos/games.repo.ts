...
/**
 * GamesRepository - Database persistence for GameStateModel
 * 
...
export class GamesRepository {
   ...
   */
  async create(gameId: string, initialState: InternalGameSnapshot, hostUserId: string): Promise<void> {
    const query = `
      INSERT INTO games (
        id, host_user_id, status, current_state, 
  ...
  /**
   * Find game by ID and reconstruct GameStateModel
   */
  ...
  /**
   * Delete game (soft delete - mark as 'deleted')
   */
  ...
  /**
   * Hard delete game (use sparingly, mainly for cleanup)
   */
  async hardDelete(gameId: string): Promise<void> {
    await this.db.query('DELETE FROM games WHERE id = $1', [gameId]);
  }
}
