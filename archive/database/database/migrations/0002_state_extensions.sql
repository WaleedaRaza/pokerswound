...
drop policy if exists hands_select_in_game on public.hands;
create policy hands_select_in_game on public.hands
  for select using (public.is_player_in_game(game_id));

-- Actions/Histories: readable if user is in the hand's game
drop policy if exists actions_select_in_game on public.game_actions;
create policy actions_select_in_game on public.game_actions
  for select using (
...
drop policy if exists histories_select_in_game on public.game_histories;
create policy histories_select_in_game on public.game_histories
  for select using (
    hand_id is null OR exists (
      select 1 from public.hands h where h.id = hand_id and public.is_player_in_game(h.game_id)
    )
...
drop policy if exists pots_select_in_game on public.pots;
create policy pots_select_in_game on public.pots
  for select using (
    exists (select 1 from public.hands h where h.id = hand_id and public.is_player_in_game(h.game_id))
  );

drop policy if exists pot_contrib_select_in_game on public.pot_contributions;
create policy pot_contrib_select_in_game on public.pot_contributions
  for select using (
    exists (
