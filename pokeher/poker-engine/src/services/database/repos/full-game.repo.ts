...
/**
 * Comprehensive repository that uses ALL schema tables correctly
 */
export class FullGameRepository {
  private db: Pool;
  ...
  async createGame(data: {
    ...
    gameType: string;
    initialState: any;
  }): Promise<{ gameUuid: string; gameTextId: string }> {
    ...
    try {
      ...
      const gamesResult = await client.query(
        ...
        RETURNING id`,
        [data.roomId, data.gameType, data.smallBlind, data.bigBlind, data.ante]
      );
      ...
      await client.query(
        `UPDATE rooms 
         ...
         WHERE id = $3`,
        [gameUuid, data.gameId, data.roomId]
      );
  ...
  async recordChipTransaction(data: {
    ...
    await this.db.query(
      `INSERT INTO chips_transactions (
    ...
    try {
      ...
      await client.query(
        `UPDATE hands 
         ...
         WHERE id = $3`,
        [data.potSize, JSON.stringify(data.communityCards), data.handId]
      );

      for (const playerResult of data.playerResults) {
        await client.query(
          `INSERT INTO player_hand_history (
        ...
        await client.query(
          `INSERT INTO player_statistics (user_id, total_hands_played, total_hands_won, updated_at)
           ...
           ON CONFLICT (user_id) DO UPDATE SET
             ...
             updated_at = NOW()`,
          [playerResult.userId, playerResult.netResult > 0 ? 1 : 0]
        );
      }
  ...
  async addPlayerToGame(data: {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO players (game_id, user_id, seat_index, stack, status)
         VALUES ($1, $2, $3, $4, 'ACTIVE')`,
        [data.gameUuid, data.userId, data.seatIndex, data.buyInAmount]
      );
