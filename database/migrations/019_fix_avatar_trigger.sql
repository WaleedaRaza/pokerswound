-- Migration 019: Fix avatar_url trigger to preserve custom avatars
-- Problem: sync_auth_user() trigger was overwriting custom avatars with Google metadata
-- Solution: Only set avatar_url from Supabase metadata on INSERT, preserve existing on UPDATE
-- 
-- Schema Context:
-- - Target table: public.user_profiles (id, username, display_name, avatar_url, ...)
...
BEGIN
  ...
  -- Check if this is a new user (INSERT) or existing user (UPDATE)
  ...
  ELSE
    ...
    UPDATE public.user_profiles SET
      ...
      -- Only update display_name if it's in metadata
...
-- Note: Trigger 'on_auth_user_created' already exists from migration 012
-- This migration only updates the function logic, not the trigger itself

SELECT 'Migration 019 completed - Avatar trigger now preserves custom avatars in user_profiles table' AS result;
