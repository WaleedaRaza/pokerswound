-- Migration 012: Final fix for foreign key constraints with Supabase auth
-- This migration properly handles all constraints and data

-- First, let's clean up any invalid data by deleting records that reference non-existent users
-- This is safer than setting to NULL for required fields

-- Delete room_seats that reference non-existent users
DELETE FROM room_seats 
WHERE user_id IS NOT NULL 
AND user_id NOT IN (SELECT id FROM auth.users);

-- Delete room_players that reference non-existent users  
DELETE FROM room_players 
WHERE user_id IS NOT NULL 
AND user_id NOT IN (SELECT id FROM auth.users);

-- Set host_user_id to NULL for rooms with invalid references (this is allowed)
UPDATE rooms 
SET host_user_id = NULL 
WHERE host_user_id IS NOT NULL 
AND host_user_id NOT IN (SELECT id FROM auth.users);

-- Clean up chips_transactions if it exists
DO $$ 
BEGIN
...
-- Now drop all existing foreign key constraints
...
-- Drop chips_transactions constraint if it exists
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'chips_transactions') THEN
...
-- Recreate chips_transactions constraint if table exists
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'chips_transactions') THEN
...
-- Insert any existing auth users into user_profiles
...
SELECT 'Migration 012 completed successfully - All auth foreign keys fixed and user profiles synced' AS result;
