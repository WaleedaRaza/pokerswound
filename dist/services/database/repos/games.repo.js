...
class GamesRepository {
    ...
    async create(gameId, initialState, hostUserId) {
        const query = `
      INSERT INTO games (
    ...
    async updateGameStateAtomic(gameId, expectedVersion, updates) {
        ...
        try {
            ...
            const lockCheck = await client.query('SELECT version FROM games WHERE id = $1 FOR UPDATE', [gameId]);
    ...
    async saveSnapshot(gameId, snapshot) {
        await this.db.query(`UPDATE games 
       SET current_state = $1, 
           status = $2,
           hand_number = $3,
           dealer_position = $4,
    ...
    async findActiveGamesByUserId(userId) {
        const result = await this.db.query(`SELECT * FROM games 
